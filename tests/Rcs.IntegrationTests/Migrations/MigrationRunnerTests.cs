...
public sealed class MigrationRunnerTests : IAsyncLifetime
{
    ...
    public async Task EditedAppliedMigrationIsDetectedAndNothingRuns()
    {
        ...
        var withNewMigration = MigrationSet.Create(
            tampered.Migrations.Select(m => (m.FileName, m.Sql)).Append(Releases.Migration(2, "must_not_run", "CREATE TABLE rcs.it_must_not_run (id integer);")));
        ...
        Assert.Equal(1L, await database.ScalarAsync<long>("SELECT count(*) FROM rcs.schema_migration"));
    ...
    {
        await database.MigrateAsync(Releases.RealPlus(Releases.Migration(2, "newer_release", "CREATE TABLE rcs.it_newer (id integer);")));
    ...
    public async Task FailingMigrationIsNotRecordedAndLaterMigrationsDoNotRun()
    {
        var release = Releases.RealPlus(
            Releases.Migration(2, "probe_a", "CREATE TABLE rcs.it_probe_a (id integer);"),
            Releases.Migration(3, "fails", "CREATE TABLE rcs.it_probe_b (id integer);\nSELECT 1 / 0;"),
            Releases.Migration(4, "after_failure", "CREATE TABLE rcs.it_probe_c (id integer);"));
        ...
        Assert.Equal(2, await database.ScalarAsync<int>("SELECT max(migration_id) FROM rcs.schema_migration"));
        ...
        Assert.Equal([3, 4], rerun.Applied.Select(applied => applied.Id));
    }
    ...
    {
        var release = Releases.RealPlus(
            Releases.Migration(2, "probe_table", "CREATE TABLE rcs.it_probe (id integer);"),
            Releases.Migration(3, "probe_index", "CREATE INDEX CONCURRENTLY IF NOT EXISTS it_probe_idx ON rcs.it_probe (id);", transactional: false));

        var result = await database.MigrateAsync(release);

        Assert.False(result.Applied.Single(applied => applied.Id == 3).Transactional);
        Assert.False(await database.ScalarAsync<bool>("SELECT transactional FROM rcs.schema_migration WHERE migration_id = 3"));
